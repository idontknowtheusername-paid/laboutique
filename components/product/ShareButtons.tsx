...
const ShareButtons: React.FC<ShareButtonsProps> = ({ productName, productUrl, productImage }) => {
  const [copied, setCopied] = useState(false);

  const shareUrl = typeof window !== 'undefined' ? window.location.href : productUrl;
  const encodedUrl = encodeURIComponent(shareUrl);
  const encodedText = encodeURIComponent(`Découvrez ${productName} sur JomionStore !`);
  ...
  const handleShare = (platform: keyof typeof shareLinks) => {
    if (platform === 'instagram' || platform === 'tiktok') {
      // Copier le lien pour Instagram et TikTok
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: productName,
          text: `Découvrez ${productName} sur JomionStore !`,
          url: shareUrl,
        });
      } catch (error) {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        <DropdownMenuItem onClick={() => handleShare('instagram')} className="cursor-pointer">
          <InstagramIcon className="w-4 h-4 mr-2 text-pink-600" />
          Instagram {copied && <span className="ml-auto text-xs text-green-600">✓ Copié</span>}
        </DropdownMenuItem>
        
        <DropdownMenuItem onClick={() => handleShare('tiktok')} className="cursor-pointer">
          <TikTokIcon className="w-4 h-4 mr-2 text-black" />
          TikTok {copied && <span className="ml-auto text-xs text-green-600">✓ Copié</span>}
        </DropdownMenuItem>
