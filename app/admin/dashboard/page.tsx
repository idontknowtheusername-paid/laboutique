...
const initialSalesData: SalesDatum[] = [];
  ...
  useEffect(() => {
    (async () => {
      ...
      if (productsRes.success && productsRes.data) {
        ...
        const derived = Object.entries(counts).map(([name, value], i) => ({
          name,
          value: Number(value),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {recentOrders.map((order) => (
                        <tr key={order.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">{order.user?.first_name} {order.user?.last_name}</td>
                          <td className="px-6 py-4 whitespace-nowrap">{order.order_items?.[0]?.vendor?.name || '—'}</td>
                          <td className="px-6 py-4 whitespace-nowrap font-bold">{formatPrice(order.total_amount)}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <Badge className={getStatusColor(order.status)}>{order.status}</Badge>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{new Date(order.created_at).toLocaleDateString('fr-FR')}</td>
                        ...
                        <tr key={user.id}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <div className="w-8 h-8 bg-beshop-primary rounded-full flex items-center justify-center">
                                <span className="text-white font-medium text-sm">{(user.first_name || user.email || '?')[0]}</span>
                              </div>
                              <div className="ml-3">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{new Date(user.created_at).toLocaleDateString('fr-FR')}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">—</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">—</td>
                          <td className="px-6 py-4 whitespace-nowrap"><Badge className="bg-green-500">Actif</Badge></td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex items-center space-x-2">
                              <Button variant="ghost" size="sm"><Eye className="w-4 h-4" /></Button>
                              <Button variant="ghost" size="sm"><Edit className="w-4 h-4" /></Button>
          ...
          <TabsContent value="vendors" className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Button variant="outline" asChild>
                  <Link href="/admin/vendors">Gérer</Link>
                </Button>
              </div>
            </div>
                      ...
                      {topVendors.map((vendor) => (
                        <tr key={vendor.id}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <div className="w-8 h-8 bg-beshop-primary rounded-full flex items-center justify-center">
                                <span className="text-white font-medium text-sm">{vendor.name.charAt(0)}</span>
                              </div>
                              <div className="ml-3">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{formatPrice(0)}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{vendor.total_orders}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{vendor.commission_rate}%</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <Badge className={getStatusColor((vendor as any).status)}>{(vendor as any).status}</Badge>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex items-center space-x-2">
                              <Button variant="ghost" size="sm"><Eye className="w-4 h-4" /></Button>
                              <Button variant="ghost" size="sm"><Edit className="w-4 h-4" /></Button>
                              <Button variant="ghost" size="sm" className="text-red-600"><Shield className="w-4 h-4" /></Button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          <TabsContent value="products" className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Button variant="outline" asChild>
                  <Link href="/admin/products">Voir tous</Link>
                </Button>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button className="bg-beshop-primary hover:bg-blue-700">Sauvegarder</Button>
                </CardContent>
              </Card>
