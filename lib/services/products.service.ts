...
export class ProductsService extends BaseService {
  ...
  /**
   * Récupérer les produits d'une catégorie pour les sections de la homepage
   */
  static async getByCategoryForHomepage(
    categoryId: string, 
    limit: number = 8
  ): Promise<ServiceResponse<Product[]>> {
    try {
      ...
      if (response.success && response.data) {
        return this.createResponse(response.data.items || []);
      } else {
        return this.createResponse([], response.error || 'Erreur lors du chargement des produits');
      }
    } catch (error) {
      return this.createResponse([], this.handleError(error));
    }
  ...
  /**
   * Récupérer les produits de plusieurs catégories pour les sections groupées
   */
  static async getByCategoriesForHomepage(
    categoryIds: string[], 
    limit: number = 8
  ): Promise<ServiceResponse<Product[]>> {
    try {
